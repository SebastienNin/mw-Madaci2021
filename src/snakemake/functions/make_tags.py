...
if os.path.isfile("../mw-Madaci2021/Sequencing_summary.xlsx"):
    samples = pandas.read_excel(
        "../mw-Madaci2021/Sequencing_summary.xlsx", sheet_name="samples", engine="openpyxl")
        ...
        if(current_df.analysis_type.unique() == "Demultiplexage_Concatenation_Quantification_QC"):
            ...
            if(len(matching_list)>0):
                ...
                for match in matching_list:
                    ...
                    SAMPLE_NAME = info_df["sample_name"]
                    ...
                    EXPECTED_CELL = info_df["Expected_cell_number"]
                    if(pandas.isna(EXPECTED_CELL)):
                    ...
                    if(info_df.Kit == "Total_seq_A"):
                        output_prefix = "out/cite-seq_count/_-cbf_1_-cbl_16_-umif_17_-umil_28_--expected_cells_" + EXPECTED_CELL + "_-o_Results_" + match + "/cat/merge-nexsteq500-pe/" + SAMPLE_NAME
                    elif(info_df.Kit == "Total_seq_B"):
                        output_prefix = "out/cite-seq_count/_-cbf_1_-cbl_16_-umif_17_-umil_28_--expected_cells_" + EXPECTED_CELL + "_-trim_10_-o_Results_" + match + "/cat/merge-nexsteq500-pe/" + SAMPLE_NAME
                    else:
                        output_prefix = "out/cite-seq_count/_-cbf_1_-cbl_16_-umif_17_-umil_28_--expected_cells_" + EXPECTED_CELL + "_-o_Results_" + match + "/cat/merge-nexsteq500-pe/" + SAMPLE_NAME
                    output_prefix = output_prefix.replace("//", "/")

                    output_target = output_prefix + "/run_report.yaml"
                    print(output_target)
                    ...
                    # No more necessary because HTO and ADT process are splitted, but I keep it for memory
                    """                 
                    ...
                    if current_df[['process']].iloc[0, 0] == "yes":
                        ...
                        mwconf["targets"].append(output_target)
